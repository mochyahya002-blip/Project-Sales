...
# Load metrics (prioritize uploaded file, else use default)
metrics_df = None
if metrics_file:
    try:
        metrics_df = pd.read_csv(metrics_file, index_col="model")
        st.toast("✓ File metrics berhasil diperbarui!", icon="✅")
    except Exception as e:
        st.error(f"❌ Gagal membaca file metrics: {str(e)}")
elif metrics_path_default:
    metrics_df = load_metrics(metrics_path_default)

# Load raw data (prioritize uploaded file, else use default)
df_raw = None
if raw_file:
    try:
        df_raw = pd.read_csv(raw_file, sep=";", encoding="utf-8-sig")
        st.toast("✓ File raw data berhasil diperbarui!", icon="✅")
    except Exception as e:
        st.error(f"❌ Gagal membaca file raw: {str(e)}")
